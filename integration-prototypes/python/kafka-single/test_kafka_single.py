...
class _FakeConsumer:
    def __init__(self, *_args, **_kwargs) -> None:
        self._records = [_FakeRecord({"id": "1", "text": "hello"})]
...
class KafkaSingleTests(unittest.TestCase):
    def test_build_message(self) -> None:
        self.assertEqual(build_message("a", "b"), {"id": "a", "text": "b"})

    def test_send_message(self) -> None:
        send_message("topic-a", {"id": "1"}, producer_factory=_FakeProducer)

    def test_consume_once(self) -> None:
        value = consume_once("topic-a", consumer_factory=_FakeConsumer)
        self.assertEqual(value, {"id": "1", "text": "hello"})
