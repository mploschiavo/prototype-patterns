...
public final class OpenSearchSingleRestServer {
    private OpenSearchSingleRestServer() {
    }

    public static HttpServer createServer(int port) throws Exception {
        HttpServer server = HttpServer.create(new InetSocketAddress(port), 0);
        server.createContext("/value", OpenSearchSingleRestServer::handle);
        return server;
    }
    ...
    private static void handle(HttpExchange exchange) {
        try {
            ...
            OpenSearchSingleRepository repository = new OpenSearchSingleRepository();
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.start();
        System.out.println("opensearch-single-rest listening on :" + port);
    }
}
